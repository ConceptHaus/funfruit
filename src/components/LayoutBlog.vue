<template lang="pug">
    section.blog
        .flex.flex-row.justify-center.items-start.my-10
            .flex.flex-col.justify-start.ml-5(class="w-2/3")
                ...
                .article.flex-col.items-center.justify-center(v-for="article in $static.allStrapiArticle.edges" :key="article.node.id")
                    g-image.object-contain.max-w-screen-md(:src="article.node.image.url" :alt="article.node.title")
                    h3.text-ui-gray.text-2xl.font-bold {{ article.node.title }}
                    p.text-ui-navy.text-sm.font-bold RECETAS
                    p.text-ui-typo.text-base(class="w-3/4") #[VueShowdown(:markdown="article.node.content | truncate(110,' ...')" flavor="github" :options="{ emoji: true }")]
                    ...
                    .border.boder-ui-typo.mt-5(class="w-3/4")
            LayoutSidebar
